...
function extractUsers(payload: any): UserApiItem[] {
  ...
  if (Array.isArray(payload?.result?.result?.users))
    return payload.result.result.users;
...
export function normalizeUser(user?: UserApiItem | null): NormalizedUser {
  return {
    ...
    fullName: user?.fullName || user?.Mini_Shin__fullName__CST || "",
...
export const getUsers = async (userId?: string) => {
  try {
    ...
    if (status === 400) {
      const response = await coreApiClient.post("/user_info_output_api", {
        input: userId ? { userId } : {},
      });
      return extractUsers(response.data);
    }
...
export const createUser = async (input: SyncUserInput) => {
  ...
  const safeMsisdn = (input.msisdn || "").trim();
...
export const syncUserToBackend = async (input: SyncUserInput) => {
  ...
  if (
    ...
    !normalizedInput.msisdn
  ) {
    throw new Error("syncUserToBackend requires userId, fullName, and msisdn");
  }

  const existingUsers = await getUsers(normalizedInput.userId).catch(() => []);
  const existingUser = existingUsers.find(
    (item) => normalizeUser(item).userId === normalizedInput.userId,
  );
